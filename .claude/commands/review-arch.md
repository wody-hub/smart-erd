...
구현이 아니라 설계를 검증하고, 잘못된 구조를 찾아내는 역할이다. 점검 대상은 `$ARGUMENTS`이다. 인자가 없으면 최근 변경 파일(`git diff --name-only` + `git ls-files --others --exclude-standard`)을 대상으로 한다.
...
아키텍처는 반드시 SOLID 원칙을 준수해야 하며, 위반해서는 안 된다. 점검 시 아래 5가지 원칙을 기준으로 위반 여부를 확인한다.

| 원칙 | 점검 기준 |
|------|----------|
| **S — 단일 책임 원칙 (SRP)** | 하나의 클래스/모듈/훅이 하나의 변경 사유만 가지는지. 여러 관심사(렌더링+동기화+직렬화 등)가 하나의 파일에 혼재하면 위반 |
| **O — 개방-폐쇄 원칙 (OCP)** | 새 기능(플러그인, 핸들러, Projector 등) 추가 시 기존 코드 수정 없이 확장 가능한지. `if-else`/`switch` 분기로 새 타입을 추가해야 하면 위반 |
| **L — 리스코프 치환 원칙 (LSP)** | 인터페이스의 구현체가 상위 타입의 계약을 준수하는지. 구현체가 예외를 던지거나 부분적으로만 동작하면 위반 |
| **I — 인터페이스 분리 원칙 (ISP)** | 클라이언트가 사용하지 않는 메서드에 의존하고 있지 않은지. 거대 인터페이스 대신 역할별 포트(Command/Query/Subscription)로 분리되어 있는지 |
| **D — 의존성 역전 원칙 (DIP)** | 상위 모듈(View, Service)이 하위 모듈(CRDT 라이브러리, DB 구현체)의 구체 타입에 직접 의존하지 않는지. 추상(인터페이스/포트)에 의존하는지 |

SOLID 위반은 **High** 심각도 이상으로 분류하며, 위반 사항에는 어떤 원칙을 어떻게 위반했는지와 구체적인 수정 방향을 포함한다.
