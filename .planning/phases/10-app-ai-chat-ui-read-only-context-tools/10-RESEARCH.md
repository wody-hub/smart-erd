...
| A2 | Member-wide TODO summaries use the user's locked policy: default own TODOs only; member-wide summaries only when the question explicitly asks and existing team/project authorization permits. | Resolved Decisions / Phase Requirements | Low-Medium: implementation still needs tests to prove no private TODO details leak. |
| A3 | Local chat retention is capped by message count rather than TTL: keep the most recent 50 messages per authenticated login/user storage key. | Resolved Decisions / Patterns | Low: exact cap can be tuned later without changing architecture. |
| A4 | Phase 10 should not replace the Phase 9 provider schema globally. Chat assembles the structured response in `AiChatExecutionService`: server facts/source chips/confirmation data come from read tools; provider `answer` maps to interpretation text only. | Resolved Decisions / State of the Art | Low-Medium: planner must make this mapping explicit across backend DTOs and frontend types. |
...
1. **Member TODO summary authorization**
   ...
   - Resolution: Phase 10 defaults to the authenticated user's own TODOs. Member-wide TODO status is allowed only when the user's question explicitly asks for team/project member TODO status and the existing team/project authorization path permits reading the project. The returned data remains summary-only by member/status/count and does not include private TODO descriptions, documents, or raw item detail unless a future phase creates a narrower authorization policy. [DECIDED: user conversation + 10-CONTEXT.md D-19 through D-21]
...
2. **Local message retention cap**
   ...
   - Resolution: Persist only the most recent 50 rendered messages per authenticated login/user storage key. No TTL is required in Phase 10. `새 대화` clears this browser-local conversation state, and logout/user switching must prevent cross-user bleed. [DECIDED: planning resolution]
...
3. **Provider response schema evolution**
   ...
   - Resolution: Do not replace the Phase 9 provider schema globally in Phase 10. `AiChatExecutionService` owns a chat-specific assembler: `confirmedFacts`, `sourceChips`, `context`, and confirmation candidates are produced from `AiReadContextService`/`AiChatContextResolver`; provider `answer` is validated by the existing `ProviderOutputValidator` and mapped to `interpretation`; `conclusion` is generated by the server assembler from the first confirmed fact/read summary or a safe localized fallback; `needsConfirmation` comes only from resolver/read-service gaps. Non-empty provider actions are rejected or omitted from the chat response. [DECIDED: planning resolution]
...
- Pitfalls: MEDIUM-HIGH - pitfalls derive from locked decisions, current code shape, and OWASP LLM risks; member TODO policy, local retention cap, and chat response assembly path are resolved above and must be enforced by tests. [VERIFIED: 10-CONTEXT.md + OWASP docs]
