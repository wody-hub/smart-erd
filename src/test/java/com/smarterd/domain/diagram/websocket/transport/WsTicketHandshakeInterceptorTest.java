...
class WsTicketHandshakeInterceptorTest {
    ...
    void beforeHandshake_storesCommonSessionAttributesOnSuccess() {
        final var interceptor = new WsTicketHandshakeInterceptor(
            validateDiagramCollaborationHandshakeUseCase
        );
        ...
        when(validateDiagramCollaborationHandshakeUseCase.validate("/ws/diagram/42", "ticket-1", 2))
            .thenReturn(Optional.of(new DiagramHandshakeValidationResult(session, 42L)));

        final var accepted = interceptor.beforeHandshake(request, response, webSocketHandler, attributes);

        assertThat(accepted).isTrue();
        assertThat(attributes.get(com.smarterd.collaboration.session.CollaborationAuthenticatedSession.SESSION_ATTR_KEY))
            .isEqualTo(session);
    ...
    void beforeHandshake_rejectsWhenTicketValidationFails() {
        ...
        when(validateDiagramCollaborationHandshakeUseCase.validate("/ws/diagram/42", "ticket-1", 1))
