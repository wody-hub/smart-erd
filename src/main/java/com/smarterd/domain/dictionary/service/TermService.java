...
public class TermService {
     ...
     * @return 용어 응답 목록
     */
    public List<TermResponse> getTerms(String loginId, Long teamId) {
        final var user = authService.findUserByLoginId(loginId);
        final var team = teamService.findTeamById(teamId);
        teamService.verifyMembership(team, user);

        return termRepository.findByTeamWithDomain(team).stream().map(TermResponse::from).toList();
    }
     ...
     * @return 용어 응답
     */
    public TermResponse getTerm(String loginId, Long teamId, Long termId) {
        final var user = authService.findUserByLoginId(loginId);
        final var team = teamService.findTeamById(teamId);
        teamService.verifyMembership(team, user);
     ...
     */
    private Domain resolveDomain(Long domainId, Team team) {
        if (domainId == null) {
            return null;
        }
