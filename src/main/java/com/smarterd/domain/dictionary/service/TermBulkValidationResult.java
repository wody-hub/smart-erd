...
/**
 * 용어 벌크 행 검증 결과를 누적하고 응답/세션 모델로 변환한다.
 */
final class TermBulkValidationResult {
    ...
    private final ArrayList<ValidatedTermRow> validRows = new ArrayList<>();
    private final ArrayList<TermBulkErrorReportRow> errorRows = new ArrayList<>();
     ...
     */
    TermBulkValidationResult(int estimatedSize) {
     ...
     */
    void addError(BulkValidationRowResult previewRow, TermBulkErrorReportRow errorReportRow) {
        errorCount++;
     ...
     * @return 검증 세션 모델
     */
    TermBulkValidationSession toSession(String loginId, Long teamId, Long setId, Instant expiresAt) {
        return new TermBulkValidationSession(
            loginId,
            teamId,
            setId,
            expiresAt,
