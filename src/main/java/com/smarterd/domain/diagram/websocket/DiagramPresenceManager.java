...
final class DiagramPresenceManager {
    ...
    record PresenceJoinResult(
        DiagramRoomManager.PresenceSnapshot snapshot,
        DiagramRoomManager.PresenceParticipant joinedParticipant,
    ...
    PresenceJoinResult onJoin(Object roomLock, Long diagramId, String userId, String displayName) {
        ...
        DiagramRoomManager.PresenceParticipant joinedParticipant = null;
        ...
        if (entry == null) {
            ...
            joinedParticipant = new DiagramRoomManager.PresenceParticipant(
                newEntry.userId,
     ...
     */
    DiagramRoomManager.PresenceSnapshot getPresenceSnapshot(Object roomLock, Long diagramId) {
        requireRoomLock(roomLock);
        final var roomState = presenceRooms.get(diagramId);
        if (roomState == null) {
            return null;
    ...
    private DiagramRoomManager.PresenceSnapshot buildPresenceSnapshot(PresenceRoomState roomState) {
        final var participants = roomState.entries
            .values()
            .stream()
            .sorted(Comparator.comparingLong((PresenceEntry e) -> e.joinSeq))
            .map((e) -> new DiagramRoomManager.PresenceParticipant(e.userId, e.displayName, e.joinSeq))
            .toList();
        return new DiagramRoomManager.PresenceSnapshot(
            roomState.roomEpoch,
