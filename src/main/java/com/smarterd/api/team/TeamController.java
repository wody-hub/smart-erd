...
public class TeamController {
    ...
    @ApiResponse(responseCode = "201", description = "팀 생성 성공", content = @Content(schema = @Schema(implementation = TeamResponse.class)))
    ...
    public ResponseEntity<TeamResponse> createTeam(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(teamService.createTeam(jwt.getSubject(), request));
    }
    ...
    @ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = TeamResponse.class)))
    @ApiResponse(responseCode = "400", description = "잘못된 요청", content = @Content)
    @GetMapping
    ...
    @ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = TeamMemberResponse.class)))
    @ApiResponse(responseCode = "400", description = "팀이 존재하지 않거나 접근 권한 없음", content = @Content)
    ...
    @ApiResponse(responseCode = "201", description = "초대 성공", content = @Content(schema = @Schema(implementation = TeamMemberResponse.class)))
    @ApiResponse(responseCode = "400", description = "잘못된 요청 (권한 없음, 이미 멤버, 사용자 미존재 등)", content = @Content)
    ...
    public ResponseEntity<TeamMemberResponse> addMember(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(teamService.addMember(jwt.getSubject(), teamId, request));
    }
     ...
     */
    @Operation(summary = "멤버 역할 변경", description = "팀 멤버의 역할을 변경한다. ADMIN 권한 필요. 소유자 역할 변경 불가.")
    @ApiResponse(responseCode = "200", description = "변경 성공", content = @Content(schema = @Schema(implementation = TeamMemberResponse.class)))
    @ApiResponse(responseCode = "400", description = "잘못된 요청 (권한 없음, 소유자 역할 변경 시도 등)", content = @Content)
    ...
    public ResponseEntity<TeamMemberResponse> updateMemberRole(
        ...
        return ResponseEntity.ok(
                teamService.updateMemberRole(jwt.getSubject(), teamId, userId, request));
    }
}
